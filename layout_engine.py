...
class Element(ABC):
    def __init__(self, width, height, nudge_x=0, nudge_y=0, padding = 0):
        self.width = width
        self.height = height
    ...
    def layout(self, x, y, max_width=0):
        ...
        子元素记录下这个位置。
        """
        self.x = x + self.padding + self.nudge_x
        self.y = y + self.padding + self.nudge_y
...
class Text(Element):
    def __init__(self, text, font, color = (0, 0, 0), padding = 0, nudge_x=0, nudge_y=0, draw_background=False, background_color=(0, 0, 0), border_radius = 16):
        ...
        super().__init__( width=width, height=height, nudge_x=nudge_x, nudge_y=nudge_y, padding=padding )
    ...
    def render(self, draw: ImageDraw.ImageDraw):
        if self.draw_background:
            ...
            bg_left = self.x - self.padding
            bg_top = self.y - self.padding
            bg_right = self.x + self.width + self.padding
            bg_bottom = self.y + self.height + self.padding
...
# 图片组件
class Image(Element):
    def __init__(self, image: PILImage.Image, width=None, height=None, nudge_x=0, nudge_y=0, padding = 0):
        if width or height :
            self.image = image.resize((int(width), int(height)), PILImage.Resampling.LANCZOS)
        else:
            self.image = image
        
        super().__init__( width=self.image.width, height=self.image.height, nudge_x=nudge_x, nudge_y=nudge_y, padding=padding )
...
class Column(Element):
    """垂直堆叠容器 (VStack) 🥞"""
    def __init__(self, children, spacing=0, align='center', padding=0,
        ...
        dynamic_height = total_children_height + total_spacing + 2 * padding
    ...
    def layout(self, x, y, max_width=None):
        ...
        if self.justify == 'space-between' and len(self.children) > 1:
            ...
            remaining_space = self.height - 2 * self.padding - total_children_height
            # 把空白平均分给孩子们中间的空隙
        ...
        for child in self.children:
            ...
            if self.align == 'center':
                offset_x = (self.width - 2 * self.padding - child.width) // 2
            elif self.align == 'right':
                offset_x = self.width - 2 * self.padding - child.width
...
class Row(Element):
    ...
    def __init__(self, children, spacing=0, align='center', padding=0,
        ...
        total_spacing = spacing * (len(children) - 1) if len(children) > 1 else 0
        dynamic_width = total_children_width + total_spacing + 2 * padding
    ...
    def layout(self, x, y, max_width=0):
        ...
        if self.justify == 'space-between' and len(self.children) > 1:
            ...
            remaining_space = self.width - 2 * self.padding - total_children_width
            # 把空白平均分给孩子们中间的空隙
            actual_spacing = remaining_space / (len(self.children) - 1)
        ...
        for child in self.children:
            ...
            if self.align == 'center':
                # 居中偏移量 = (容器可用高度 - 孩子高度) / 2
                offset_y = (self.height - 2 * self.padding - child.height) // 2
            elif self.align == 'bottom':
                offset_y = self.height - 2 * self.padding - child.height
